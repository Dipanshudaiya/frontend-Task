...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await login(form);
      // navigation is handled inside try, which is good
      navigate("/dashboard");
    } catch (err) {
      console.error("Login Error:", err);
      // No alert here if we want it to be smoother, but user might want feedback
      alert("Login failed. Please check your credentials.");
    } finally {
      // Small delay before enabling to prevent rapid double-clicks if it fails fast
      setLoading(false);
    }
  };
